# ¡Hola! Soy Lucy Aracelly Melo Martínez 👋  

###👩‍💻 Soy Analista de Datos de HENRY bootcamps
#### Odontóloga especialista en Administración en Salud.

---

### 📊 Sobre mí  
- Analista de Datos formada en el Bootcamp de **Soy Henry**, con enfoque en análisis, visualización y toma de decisiones basadas en datos.  
- Más de 20 años de experiencia en el sector salud como odontóloga general, coordinadora de programas de prevención y docente en instituciones educativas del área de la salud.  
- Me apasiona transformar datos en información útil para optimizar procesos, entender comportamientos y mejorar la toma de decisiones en el ámbito sanitario y administrativo.  
- Disfruto aprender nuevas herramientas, trabajar en equipo y compartir conocimientos con otros.

---

### 🛠️ Tecnologías y Herramientas  

**Lenguajes:**  
- Python (Pandas, NumPy, Matplotlib, Seaborn)  
- SQL

**Visualización:**  
- Power BI  
- Excel  
- Google Sheets  

**Bases de Datos:**  
- SQL Server (consultas, joins, funciones)

**Control de Versiones:**  
- Git • GitHub  

**Metodologías:**  
- Agile • Scrum

---

### 🎯 Objetivo  
Busco oportunidades en **Data Analytics** donde pueda aplicar mis habilidades técnicas, mi experiencia en el sector salud y mi capacidad para analizar información y presentar insights que apoyen la toma de decisiones.

---

### 📂 Proyectos Destacados  

#### 📌 *Decisiones Inteligentes NBA: Análisis de Talento y Rendimiento*  
**Rol:** Data Analyst  
**Descripción:** Análisis de variables clave que influyen en las victorias de equipos de la NBA desde una perspectiva de talento humano.  
- Limpieza, análisis exploratorio y modelado básico de datos.  
- Visualización de insights y métricas de desempeño.  
- Construcción del dashboard final.  
**Stack:** Python • Pandas • Seaborn • Power BI  
[🔗 •	https://github.com/MelRossi/NBA-analisis-rendimiento  ]



---

### 🌐 Contacto  
📍 Popayán, Colombia  
📧 lucymelo.08@gmail.com  
💼 www.linkedin.com/in/lucy-aracelly-melo-martínez-823b5b159  
